<template>
        ...
        <div class="py-2">
          <div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8 overflow-hidden bg-white shadow-sm sm:rounded-lg">
            ...
            <table v-else class="w-full border text-sm">
              ...
              <tbody>
                <tr v-for="user in nonAdmins" class="hover:bg-gray-50">
                  <td class="p-2 border-b">{{ user.name }}</td>
                  <td class="p-2 border-b">{{ user.email }}</td>
                  <td class="p-2 border-b">{{ new Date(user.created_at).toLocaleDateString() }}</td>
                  <td class="p-2 border-b">
  ...
  const nonAdmins = props.users.filter((user) => {
    return user.is_admin === 0 && user.isVerified === 1;
  })
  ...
  function makeAdmin(userId) {
    if (confirm('Weet je zeker dat je deze gebruiker een Admin wilt maken?')) {
      router.post(`/admin/users/${userId}/makeadmin`, {}, {
        preserveState: false,
      }) 
    }
  }
